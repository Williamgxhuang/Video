...
正式作品已经单独放在 `projects.js` 中。以后通常不需要修改 `index.html`。

在 GitHub 仓库中：

1. 打开 `projects.js`。
2. 点击右上角铅笔图标 `Edit this file`。
3. 找到对应项目，修改标题、说明、封面和视频地址。
4. 点击 `Commit changes`。
...
```js
{
  ...
  category: "motion",
  label: "品牌影像",
  description: "Momenta 品牌视频与发布会内容。",
  cover: "assets/covers/smart-road.webp",
  video: "https://github.com/Williamgxhuang/Video/releases/download/media-v1/smart-road.mp4"
}
```

### 上传封面

仓库中进入 `assets/covers/`，上传封面图片，例如：

```text
assets/covers/smart-road.webp
```
...
推荐 16:9、WebP/JPG，尽量控制在 1 MB 以内。

### 填写视频

视频放进 GitHub Release 后，复制 `.mp4` 附件链接，例如：

```text
https://github.com/Williamgxhuang/Video/releases/download/media-v1/smart-road.mp4
```
...
当 `cover` 留空且 `video` 是 B站 BV/AV 链接时，页面会自动尝试读取 B站封面；如果平台图片接口受跨域限制，则退回 B站官方站外播放器的封面画面。手动填写 `cover` 时始终优先使用自己的封面。建议使用包含 `BV...` 或 `av...` 的完整视频地址，不要只填 `b23.tv` 短链。
...
右上角的“临时预览”只用于当前浏览器测试，不会写回 GitHub，也不会永久保存。
